...
{  
    char* TPX_name;
    string oldSuffix = "tpx3";
    string newSuffix = "txt";
    string TPX_name_oo;
	...
	TPX_name_oo = argv[1];
        ...
        size_t len = TPX_name_oo.length();
	cout << "file dim = " << len << endl;
        TPX_name_oo.replace(TPX_name_oo.length() - oldSuffix.length(), oldSuffix.length(), newSuffix);
	cout << "imported file new: " << TPX_name_oo << endl;
    }

    //ofstream xy_file("converted_butbetter.txt");   //Converted and saved txt file
    ofstream xy_file(TPX_name_oo);   //Converted and saved txt file
    streampos begin, end;
	...
	double prev_TDC_timestamp = 0.0; // Holds the previous TDC timestamp
	int rollover_counter = 0;        // Counter for the number of trigger rollovers
	double prev_global_timestamp = 0.0; // Holds the previous pixel timestamp
	int global_counter = 0;        // Counter for the number of pixel rollovers
        ...
        for (unsigned long long i = 0; i < NumofPacket; i++) {
            ...
            if (HeaderBuffer[0] == 'T' && HeaderBuffer[1] == 'P' && HeaderBuffer[2] == 'X') {
                ...
                for (int j = 0; j < size / 8; j++) {
                    ...
                    // Check for timestamp rollover and adjust the TDC timestamp
                    if (TDC_timestamp < prev_TDC_timestamp) {
                        rollover_counter++;  // Increment the rollover counter
                    }

                    // Save the current TDC timestamp as the previous one for the next iteration
                    prev_TDC_timestamp = TDC_timestamp;

                    // Add the rollover adjustment
                    TDC_timestamp += rollover_counter * time_unit * (1ULL<<44) * 1E-9;


						//uncomment below to save TDC timestamps into the txt file                        
                    if ((temp >> 56) == 0x6f) xy_file << setprecision(15) << TDC_timestamp << "  -1"  <<  "  -1"  <<  "  -1" << endl;  
                        //cout<< "TDC timestamp: " << setprecision(15) << TDC_timestamp << endl;
			numofTDC=numofTDC+1;												
                        break;

						//uncomment below to save TDC timestamps into the txt file                        
                    if ((temp >> 56) == 0x6e) xy_file << setprecision(15) << TDC_timestamp << "  -2"  <<  "  -2"  <<  "  -2" << endl;  
                        //cout<< "TDC timestamp: " << setprecision(15) << TDC_timestamp << endl;
			numofTDC=numofTDC+1;												
                        break;
                    ...
                    // Check for timestamp rollover and adjust the TDC timestamp
                    if (global_timestamp < (prev_global_timestamp - 20E9)) {
                        global_counter++;  // Increment the pixel rollover counter (can maybe miss part of hit split by rollover, fix later maybe (not important))
                    }
		    ...
		    if (global_timestamp > (prev_global_timestamp + 20E9)) {
                    ...
                    // Save the current TDC timestamp as the previous one for the next iteration
                    prev_global_timestamp = global_timestamp;

                    // Add the rollover adjustment
                    global_timestamp += global_counter * time_unit * (1ULL<<42);
			...
			if (((global_timestamp / 1E9) - (prev_TDC_timestamp + rollover_counter * time_unit * (1ULL<<44) * 1E-9)) < 0.0001) {
			...
			if ((i % 100000) == 0) {
				//cout << i << " out of " << NumofPacket << " number of lines " << endl;
			}
			else if (((i % 100000) > 1) & ((i % 100000) < 100)){
				cout<< setprecision(15) << global_timestamp / 1E9 << "  ToT: " << TOTns << " x: " << x << " y: " << y << endl;
			}
                ...
                i += (size / 8);
		if ((i%10) == 0) printf("i : %lld over %lld \r", i, NumofPacket);
            }
        }
